...
public  abstract class clsAVLTree {
	...
	public clsAVLNode rotacionDoble(clsAVLNode node, boolean esIzquierda){
		if (esIzquierda){
			// rotacion simple derecha sobre el hijo izquierdo del nodo desequilibrado
			// ahora rotacion simple izquierda sobre el nodo desequilibrado
			node.setLeftChild(rotacionSimple(node.getLeftChild(),false));
			return rotacionSimple(node, true);
				
		}else {		  

			// rotacion simple izquierda sobre el hijo derecho del nodo desequilibrado
			// rotacion simple derecha sobre el nodo desequilibrado
	...
	private static int height(clsAVLNode node){
		if (node==null)
			return -1;
		else
